...
def main():
    ...
    parser.add_argument('--prompt', nargs='+', type=str, help='Input prompt(s)')
    parser.add_argument('--negative_prompt', nargs='*', type=str, help='Negative prompt(s)', default=[])
    ...
    grad_ctx = torch.inference_mode() if not args.use_guidance else nullcontext()
    ...
    with grad_ctx:
        ...
        negative_prompts: list[str] = args.negative_prompt
        ...
        log.info(f'Negative prompts: {negative_prompts}')

        # validate negative prompts length
        bs = len(prompts)
        if len(negative_prompts) == 0:
            negative_prompts = [''] * bs
        elif len(negative_prompts) == 1 and bs > 1:
            negative_prompts = negative_prompts * bs
        elif len(negative_prompts) != bs:
            raise ValueError(f"--negative_prompt count ({len(negative_prompts)}) must match --prompt count ({bs})")
        ...
        for i in range(bs):
            audio_i = audios[i]
            if video_path is not None:
                safe_prompt = prompts[i].replace(' ', '_').replace('/', '_').replace('.', '')
                save_path = output_dir / f'{video_path.stem}_{i}_{safe_prompt}.flac'
            else:
                safe_prompt = prompts[i].replace(' ', '_').replace('/', '_').replace('.', '')
                save_path = output_dir / f'{safe_prompt}.flac'
            torchaudio.save(save_path, audio_i, seq_cfg.sampling_rate)
            log.info(f'Audio saved to {save_path}')

            if video_path is not None and not skip_video_composite:
                video_save_path = output_dir / f'{video_path.stem}_{i}_{safe_prompt}.mp4'
                make_video(video_info, video_save_path, audio_i, sampling_rate=seq_cfg.sampling_rate)
